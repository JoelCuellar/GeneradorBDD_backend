...
export interface CreateDomainAttributeInput {
  projectId: string;
  actorId: string;
  classId: string;
  name: string;
...
@Injectable()
export class CreateDomainAttributeUseCase {
  constructor(
    @Inject(DOMAIN_MODEL_REPOSITORY)
    private readonly repo: DomainModelRepository,
  ) {}

  async execute(input: CreateDomainAttributeInput) {
    ...
    const config = input.config ? this.validateConfig(input.config) : null;
    ...
    return this.repo.createAttribute(payload);
  }
